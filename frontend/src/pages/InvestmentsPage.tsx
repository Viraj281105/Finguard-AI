      ...
      <nav className="sticky top-0 z-50 border-b border-white/[0.05] bg-[#080a0e]/80 backdrop-blur-xl px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className="px-4 py-2 text-sm bg-[#a78bfa] text-black font-semibold rounded-xl
                       hover:bg-[#9061f9] transition-colors"
          >
            + Add
          </button>
        </div>
          ...
          <div className="flex gap-3">
            ...
            <div className={`px-4 py-2 rounded-xl border ${
              ...
              <p className={`text-xs uppercase tracking-widest ${
                overallGain >= 0 ? "text-emerald-400/60" : "text-rose-400/60"
              }`}>P&L</p>
              <p className={`text-sm font-semibold ${
                overallGain >= 0 ? "text-emerald-400" : "text-rose-400"
              }`}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  {["Name", "Type", "Invested", "Current Value", "P&L", ""].map((h) => (
                    <th
                      key={h}
                      className={`px-6 py-4 text-xs text-white/30 uppercase tracking-widest font-medium ${
                        h === "" ? "" : h === "Name" ? "text-left" : "text-right"
                      }`}
                    >
                      {h}
                    </th>
              ...
              <tbody>
                {investments.map((inv) => {
                  ...
                  const gainP = inv.amountInvested > 0
                    ? ((gain / inv.amountInvested) * 100).toFixed(2)
                    : "0.00";
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-4">
                        ...
                        {inv.notes && (
                          <p className="text-xs text-white/30 mt-0.5">{inv.notes}</p>
                        )}
                      </td>
